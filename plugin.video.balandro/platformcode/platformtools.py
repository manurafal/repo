...
def render_items(itemlist, parent_item):
    ...
    if parent_item.channel != 'tracking' and itemlist[0].contentType in ['movie', 'tvshow']: #, 'season', 'episode'
...
def set_context_commands(item, parent_item, colores):
    ...
    for command in context:
        ...
        if type(command) == dict:
            ...
            titulo = command['title'] if parent_item.channel != 'tracking' else '[COLOR %s]%s[/COLOR]' % (colores['tracking'], command['title'])
    ...
    # Guardar seguimiento
    if item.contentType in ['movie', 'tvshow', 'season', 'episode'] and parent_item.channel not in ['tracking', 'downloads']:
    ...
    if item.contentType in ['movie', 'tvshow']: # and parent_item.channel != 'search':
        ...
        item_search = Item(channel='search', action='search', buscando=buscando, search_type=item.contentType, from_channel='')
        context_commands.append( ('[COLOR %s]Buscar parecido %s[/COLOR]' % (colores['search_similar'], tipo_busqueda), config.build_ContainerUpdate(item_search)) )
...
def formatear_enlace_play(item, colorear=False, colores={}):
    if item.server == '' or (item.server != '' and item.quality == '' and item.language == '' and item.age == '' and item.other == ''):
        mantener_titulo = True
    else:
        mantener_titulo = False

    if not colorear:
        titulo = item.title if mantener_titulo else item.server.capitalize()
        ...
        titulo = '[COLOR %s]%s[/COLOR]' % (colores['server'], item.title if mantener_titulo else item.server.capitalize())
    ...
    if item.verified:
        if item.verified == 0: vcolor = 'white'; vtext = '?'
        elif item.verified == -1: vcolor = 'gray'; vtext = 'ko'
        else: vcolor = 'gold'; vtext = 'ok'
        titulo = '[COLOR %s](%s)[/COLOR] ' % (vcolor, vtext) + titulo
...
def play_from_itemlist(itemlist, parent_item):
    ...
    # ~ for it in itemlist: logger.info(it)
    from core import servertools
    itemlist = servertools.filter_and_sort_by_server(itemlist)
    itemlist = servertools.filter_and_sort_by_language(itemlist)

    # Verificación de enlaces según configuración
    if len(itemlist) > 0 and config.get_setting('comprueba_enlaces', default=False):
        itemlist = servertools.check_list_links(itemlist, 
                                                config.get_setting('comprueba_enlaces_num', default=3),
                                                config.get_setting('comprueba_enlaces_discard', default=False) )
    ...
    if autoplay and len(itemlist) > 0 and itemlist[0].channel in autoplay_channels_discarded: autoplay = False
    ...
    if autoplay:
        ...
        if not ok_play:
            if esperar_seleccion: # si se ha llegado al límite de enlaces a intentar y todavía quedan, se muestra diálogo para selección del usuario
                ...
                play_fake()
                dialog_ok(config.__addon_name, 'No se ha podido reproducir ninguna de las opciones')
        else:
            dialog_notification('Autoplay resuelto', it.title)
...
def play_video(item, parent_item, autoplay=False):
    ...
    if item.video_urls:
        ...
        from core import servertools
        video_urls, puedes, motivo = servertools.resolve_video_urls_for_playing(item.server, item.url, video_password=parent_item.url)
