...
def mainlist_pelis(item):
    ...
    itemlist.append(item.clone( title='Por letra (A - Z)', action='mapa', url=host + '/mapa-de-peliculas', extra="peliculas" ))

    itemlist.append(item.clone( title='Sección Adultos +18', action='peliculas', url=host + '/catalogue?type=adultos' ))
...
def mainlist_series(item):
    ...
    itemlist.append(item.clone( title='Por letra (A - Z)', action='mapa', url=host + '/mapa-de-series', extra="series" ))
    ...
    for match in bloque:
        ...
        for scrapedurl, scrapedtitle, scrapedthumbnail, year, plot in matches:
            ...
            if "/serie/" in scrapedurl:
                ...
                filter_thumb = limpiar_thumbnail(scrapedthumbnail)
    ...
    for match in bloque:
        if item.extra == "mapa":
            ...
            for idiomas, calidad, scrapedthumbnail, scrapedurl, scrapedtitle in matches:
                ...
                if calidad: calidad = calidad.capitalize().replace("Hd", "HD")
                audios = []
                ...
                itemlist.append(item.clone( action="findvideos", title=scrapedtitle, url=url,
                                            thumbnail=scrapedthumbnail, contentType="movie", contentTitle=scrapedtitle,
                                            qualities=calidad, languages=", ".join(audios), infoLabels={'filtro':filter_list} ))
    ...
    if tipo_enlaces == 'Online':
        ...
        for scrapedopcion, scrapedlanguage, scrapedcalidad in matches:
            ...
            itemlist.append(Item(channel = item.channel, action = 'play',
                                 ...
                                 language = language, quality = scrapedcalidad.strip(), other = other
                           ))
    ...
    if matches:
        for scrapedurl, idioma, server, calidad in matches:
            ...
            itemlist.append(Item(channel = item.channel, action = 'play',
                                 ...
                                 language = IDIOMAS[idioma], quality = calidad, other = other
                           ))
