...
def mainlist_pelis(item):
    ...
    itemlist.append(item.clone( title = 'Lista de películas', action = 'list_all', url = host_catalogue ))
...
def findvideos(item):
    ...
    for url, language, quality in matches:
        ...
        itemlist.append(Item( channel = item.channel, action = 'play', server = server,
                              title = '', url = url,
                              language = IDIOMAS.get(language, language), quality = quality.strip()
                       ))
