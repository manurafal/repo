...
def list_all(item):
    ...
    if item.search_post:
        data = httptools.downloadpage(item.url, post=item.search_post).data
    else:
...
def episodios(item):
    ...
    data = jsontools.load(httptools.downloadpage('https://hdfull.me/a/episodes', post=post).data)
    # ~ logger.debug(data)
    ...
    for epi in data:
        ...
        thumb = 'https://hdfull.me/tthumb/220x124/' + epi['thumbnail']
...
def findvideos(item):
    ...
    data_js = httptools.downloadpage(host+'/js/providers.js').data
    try:
