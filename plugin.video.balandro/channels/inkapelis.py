...
def mainlist_pelis(item):
    ...
    # ~ itemlist.append(item.clone( title = 'Castellano', action = 'list_all', url = host + '?idioma=Castellano&s=' ))
    # ~ itemlist.append(item.clone( title = 'Latino', action = 'list_all', url = host + '?idioma=Latino&s=' ))
    # ~ itemlist.append(item.clone( title = 'VOSE', action = 'list_all', url = host + '?idioma=Subtitulada&s=' ))
...
def anyos(item):
    ...
    for anyo in range(int(datetime.today().year), 1970, -1):
        itemlist.append(item.clone( title=str(anyo), url=host + 'year_relase/' + str(anyo), action='list_all' ))
...
def calidades(item):
    ...
    opciones = ['Dvdrip', 'Blurayrip'] #TODO completar...
    for opc in opciones:
...
def findvideos(item):
    ...
    data = httptools.downloadpage(item.url).data
    # ~ logger.debug(data)
    # ~ data = scrapertools.find_single_match(data, '<div class="table-responsive dlmt" id="olmt">(.*?)</table>') # descartar descargas ?
