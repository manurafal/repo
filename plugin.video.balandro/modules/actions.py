...
def show_help_settings(item):
    ...
    txt += '[CR][CR]'
    txt += '*) En "Reproducción" los parámetros para [COLOR gold]verificar enlaces[/COLOR] sirven para comprobar si estos siguen vigentes, ya que es bastante habitual que algunos enlaces dejen de funcionar.'
    txt += ' Como esta verificación adicional puede ralentizar la respuesta se permite configurar si hay que hacerla y para cuantos enlaces.'

    txt += '[CR][CR]'
    txt += '*) Otra opción que puede provocar una demora en los tiempos de respuesta es en "Otros/TMDB" si se activa "[COLOR gold]buscar información extendida[/COLOR]".'
...
def show_help_tips(item):
    ...
    txt += ' Si dispones de un teclado puedes acceder a él pulsando la tecla C y en dispositivos táctiles manteniendo pulsado un elemento.'
    txt += ' Si solamente usas un mando de TV es recomendable configurar una de sus teclas con "ContextMenu".'
...
def show_help_faq(item):
    ...
    txt += ' De momento no hay preferencias para calidades ya que es un valor no demasiado fiable ni uniforme en el conjunto de canales.'
