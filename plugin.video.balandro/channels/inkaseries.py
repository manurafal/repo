...
def findvideos(item):
    ...
    patron += '.*?<span class="num">([0-9-]+)</span>'
    matches = re.compile(patron, re.DOTALL).findall(data)
    for url, servidor, language, quality, votos in matches:
        language = language.replace('\n', '').strip()

        itemlist.append(Item( channel = item.channel, action = 'play', server = servidor.lower(),
                              title = '', url = url,
                              language = IDIOMAS.get(language, language), quality = quality, other = votos if '-' in votos or votos == '0' else '+'+votos
                       ))
