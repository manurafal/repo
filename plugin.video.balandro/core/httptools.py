...
def downloadpage(url, post=None, headers=None, timeout=None, follow_redirects=True, cookies=True, replace_headers=False,
    ...
    if type(response['code']) == int and response['code'] > 399 and raise_weberror:
        ...
        if lanzar_error:
            ...
            is_channel = str(is_channel).replace("/servers/","\\servers\\")  # Para tener en cuenta distintos sistemas operativos
            if "\\servers\\" in is_channel:
                lanzar_error = False
    ...
    if use_cache and type(response['code']) == int and response['code'] >= 200 and response['code'] < 400 \
       and response['data'] != '' and 'ERROR 404 - File not found' not in response['data']:
