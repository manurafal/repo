...
def host_by_lang(lang=''):
    ...
    if lang == 'Lat': return 'http://www.cinecalidad.to/'
    if lang == 'Esp': return 'http://www.cinecalidad.to/espana/'
    if lang == 'Por': return 'http://www.cinemaqualidade.to/'
    return 'http://www.cinecalidad.to/'
...
def findvideos(item):
    ...
    for srv, encoded in matches:
        if srv in server_url:
            ...
            itemlist.append(Item(channel = item.channel, action = 'play', server = srv.lower(),
                                 title = '', url = url,
