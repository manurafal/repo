...
def downloadpage_proxy(url, use_cache=False):

    proxies = config.get_setting('proxies', 'gnula', default='').replace(' ', '')
    ...
    for proxy in proxies:
        ...
        resp = httptools.downloadpage(url, use_proxy=use_proxy, use_cache=use_cache, raise_weberror=False)
        if type(resp.code) == int and resp.code < 200 or resp.code > 399:
            ...
            break
    if not proxy_ok and use_proxy != None: 
        txt = 'Ninguno de los proxies ha funcionado.' if len(proxies) > 1 else 'El proxy no ha funcionado.'
        platformtools.dialog_notification('Sin respuesta válida', txt)
        return ''
    else:
        return resp.data
...
def generos(item):
    ...
    # ~ data = httptools.downloadpage(host + 'generos/lista-de-generos/').data
    data = downloadpage_proxy(host + 'generos/lista-de-generos/', use_cache=True)
    # ~ logger.debug(data)

    patron = '<td>\s*<strong>([^<]+)</strong>\s*\[<a href="([^"]+)" target="_blank"'
    matches = re.compile(patron, re.DOTALL).findall(data)
    for title, url in matches:
        itemlist.append(item.clone( title=title, url=url, action='list_all' ))

    patron = '<td>\s*<strong>([^<]+)</strong>\s*\[<a href="([^"]+)" title="([^"]+)" target="_blank"'
    matches = re.compile(patron, re.DOTALL).findall(data)
...
def list_all(item):
    ...
    data = downloadpage_proxy(item.url, use_cache=True)
    # ~ logger.debug(data)
...
def findvideos(item):
    ...
    # ~ data = httptools.downloadpage(item.url).data
    data = downloadpage_proxy(item.url, use_cache=False)
    # ~ logger.debug(data)
