...
#TODO añadir otras webs con lista de proxies gratuítos
# https://hidemyna.me/es/proxy-list/
# http://spys.one/en/

opciones_provider = ['proxy-list.download', 'proxyscrape.com', 'lista_proxies.txt']
opciones_tipo = ['Cualquier tipo', 'Elite', 'Anonymous', 'Transparent']
opciones_pais = ['Cualquier país', 'ES', 'US', 'FR', 'DE', 'CZ', 'IT', 'CH', 'MX', 'RU'] # TODO completar...


# Parámetros proxytools_ específicos del canal
def get_settings_proxytools(canal):
    provider = config.get_setting('proxytools_provider', canal, default='proxy-list.download')
...
def _buscar_proxies(canal, url):
    ...
    # ---------------------------------
    if config.get_setting('proxytools_log', default=True):
